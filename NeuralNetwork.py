...
class NeuralNetwork():
    def __init__(self,
        ...
        # "random.rand" function creates an array of the given shape and fill it with 
    ...
    def forward_pass(self, inputs):
        ...
        # If a forward pass has occured before (i.e., bias term has been appended to y_hidden), 
        # then we have to remove the bias from the hidden neurons
        ...
        for i in range(self.n_hidden):
            ...
            # pass the linear combination of the inputs to the activation function            
    ...
    def backward_pass(self, inputs, targets, learning_rate):
        ...
        for i in range(self.n_out):
            ...
            #this is used when targets value is a Scalar for example xorExample.py code
            if np.isscalar(targets): 
                output_deltas[i] = (outputs[i] - targets) * sigmoid_derivative(outputs[i])
                
            #this is used when targets value has 3 classes for example irisExample.py code
        ...
        for i in range(len(hidden_deltas)):
            ...
            hidden_weight_err = 0.0
            for j in range(len(output_deltas)):
                hidden_weight_err = hidden_weight_err + self.w_out[i,j]* output_deltas[j]
            
            hidden_deltas[i] = sigmoid_derivative(self.y_hidden[i]) * hidden_weight_err
            ########################################/
        ...
        for i in range(len(self.y_hidden)):
            for j in range(len(output_deltas)):
                ...
                self.w_out[i,j] =  self.w_out[i,j] - (learning_rate*self.y_hidden[i] * output_deltas[j])
                ########################################/
        ...
        for i in range(len(inputs)):
            for j in range(len(hidden_deltas)):
                ...
                self.w_hidden[i,j] = self.w_hidden[i,j] - (learning_rate * inputs[i] * hidden_deltas[j])
                ########################################/
